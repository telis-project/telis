...
beforeEach(function () {
    ...
    Cache::forget('github_stars_Telis_telis');
});

it('gets stars count from GitHub API', function () {
    // Mock HTTP response
    Http::fake([
        'api.github.com/repos/Telis/telis' => Http::response([
            'stargazers_count' => 125,
        ], 200),
    ]);

    // Call the service
    $result = $this->service->getStarsCount('Telis', 'telis');
    ...
    Http::assertSent(function ($request) {
        return $request->url() === 'https://api.github.com/repos/Telis/telis' &&
...
it('uses cached stars count on subsequent calls', function () {
    ...
    $firstResult = $this->service->getStarsCount('Telis', 'telis');
    ...
    $secondResult = $this->service->getStarsCount('Telis', 'telis');
...
it('returns 0 when API call fails', function () {
    ...
    Http::fake([
        'api.github.com/repos/Telis/telis' => Http::response(null, 500),
    ]);
...
it('formats small numbers as plain numbers', function () {
    ...
    // Call the service
    $result = $this->service->getFormattedStarsCount('Telis', 'telis');
